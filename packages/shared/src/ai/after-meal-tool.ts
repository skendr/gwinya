import { isoDay } from "../format/dates";
import type { SymptomLog } from "../domain/types";

/**
 * The OpenAI Realtime function the voice companion calls to persist the
 * after-meal check (see lib/ai/companion.ts and the meal companion clients on
 * web + mobile).
 *
 * The fields mirror the `afterMeal` checklist items (content/checklists.ts)
 * and map onto a SymptomLog row exactly like the tap form — so a meal checked
 * by voice and one tapped in are indistinguishable in the symptom_logs table.
 *
